...
export const useGrowthSystem = () => {
  ...
  // ========================================
  // DRAFT REGENERATION (via serverless /api/regenerate-draft)
  // ========================================
  const regenerateDraft = useCallback(async (draft) => {
    if (!draft?.id) {
      throw new Error('Draft ID is required for regeneration.');
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.message || 'Error regenerando borrador');
      }

      const data = await response.json();

      if (!data.success || !data.result) {
        throw new Error('No se pudo regenerar el borrador.');
      }
